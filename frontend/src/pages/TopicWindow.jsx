...
export default function TopicWindow() {
  ...
  const loadSession = () => {
    try {
      const loadedSession = sessionService.getActiveSession();
      if (loadedSession && loadedSession.id === sessionId) {
        setSession(loadedSession);
        setSessionTime(loadedSession.duration || 0);
        
        // Load YouTube video if present
        if (loadedSession.youtubeUrl) {
          const embedUrl = youtubeService.getEmbedUrl(loadedSession.youtubeUrl);
          const videoContent = {
            id: 'session-video',
            type: 'youtube',
            url: embedUrl,
            title: loadedSession.workspaceName || 'YouTube Video',
          };
          setContents([videoContent]);
          setCurrentContent(videoContent);
          setCurrentVideo(embedUrl);
        }
      } else {
        console.error('Session not found or mismatch');
        navigate('/dashboard');
      }
    } catch (err) {
  ...
  const handleStartBreak = () => {
    if (!sessionId) return;
    try {
      sessionService.startBreak(sessionId);
      setIsOnBreak(true);
      // Default break duration: 10 minutes for standard, 20 for recommended
      const breakDuration = session?.sessionType === 'recommended' ? 1200 : 600;
  ...
  const handleEndBreak = () => {
    if (!sessionId) return;
    try {
      sessionService.endBreak(sessionId);
      setIsOnBreak(false);
      setBreakTimeLeft(0);
    } catch (err) {
  ...
  const handleCompleteSession = async () => {
    ...
    try {
      // Generate quiz based on the workspace/session name
      const topic = session?.workspaceName || 'General Knowledge';
      
      console.log(`Generating 20 questions for topic: ${topic}`);
      
      // Generate quiz from the session topic
      const quiz = await quizService.generateQuizFromTopic(topic, 20);
      
      // Complete session and store quiz
      sessionService.completeSession(sessionId, quiz);
      
      // Navigate to dashboard
      navigate('/dashboard', {
        state: { 
          message: 'Session complete! Your test is ready.',
          sessionId: sessionId
        }
      });
    } catch (err) {
  ...
  const handleAddYouTube = () => {
    if (youtubeUrl.trim()) {
      ...
      if (!youtubeService.isValidYouTubeUrl(youtubeUrl)) {
        setDocError('Please enter a valid YouTube URL');
        return;
      }

      const embedUrl = youtubeService.getEmbedUrl(youtubeUrl);
      const newContent = {
        id: Date.now(),
        type: 'youtube',
        url: embedUrl,
            ...
            {sessionId && (
              <>
                ...
                <button
                  ...
                  disabled={completing || generatingQuiz}
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {generatingQuiz ? (
                    <>
